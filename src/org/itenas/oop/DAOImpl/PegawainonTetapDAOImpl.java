...
public class PegawainonTetapDAOImpl implements PegawainonTetapDAO{
	...
	public List<PegawainonTetap> GetAllPegawainonTetap() {
		...
		try {
			...
			while (rs.next()) {
				...
				pnt.setIDP(rs.getObject(1).toString());
	...
	public void SavePegawainonTetap(PegawainonTetap pnt) {
		...
		try {
			db.connect();
			query = "INSERT INTO pegawainontetap (IDP, Nama_Pegawai, Divisi, Level, Masa_Kerja, Status, Gaji_Pokok, Jumlah_Lembur, Jumlah_Cuti, Jumlah_Absen, Jumlah_Dinas)"
				  + "VALUES ('" + pnt.getIDP() +"', '"
				  + pnt.getNama_Pegawai() +"', '"
				  + pnt.getDivisi() +"', '"
				  + pnt.getLevel() +"', "
