...
public class displayPegawaiTetap {
	...
	public static void DisplayPegawaiTetap(List<PegawaiTetap> listPegawaiTetap) {
		...
		for (PegawaiTetap pt : listPegawaiTetap) {
			System.out.println("    |\t"+pt.getIDP()+"\t|\t"+pt.getNama_Pegawai()+"\t|\t"+pt.getDivisi()+"\t|\t"+pt.getLevel()+"\t|\t"+pt.getMasa_Kerja()+"\t\t|\t"+pt.getStatus()+"\t\t|\t"+pt.getGaji_Pokok()+"\t\t|\t"+pt.getJumlah_Lembur()+"\t\t|\t"+pt.getJumlah_Cuti()+"\t\t|\t"+pt.getJumlah_Absen()+"\t\t|\t"+pt.getJumlah_Dinas()+"\t\t|");
			System.out.println("     ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		}
	...
	public static void FormSlipGaji(List<PegawaiTetap> listPegawaiTetap) {
		...
		for (PegawaiTetap pt : listPegawaiTetap) {
			if(IDP.equals(pt.getIDP())){
				...
				int potongan = pt.getJumlah_Cuti() * 150000;
				double pajak = pt.getGaji_Pokok() * 0.1;
				System.out.println("              *************************************                  ");
		        ...
		        System.out.println("              | Total Gaji		: Rp. " +((pt.getGaji_Pokok() + bonus) - potongan - (int)pajak));
			}
		}
		System.out.print("[B] Tekan Tombol B untuk kembali ke menu sebelumnya: ");
		back = s.next().charAt(0);
