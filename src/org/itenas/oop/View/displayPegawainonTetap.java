...
public class displayPegawainonTetap {
	...
	public static void DisplayPegawainonTetap(List<PegawainonTetap> listPegawainonTetap) {
		...
		for (PegawainonTetap pnt : listPegawainonTetap) {
			System.out.println("    |\t"+pnt.getIDPnT()+"\t|\t"+pnt.getNama_Pegawai()+"\t|\t"+pnt.getDivisi()+"\t|\t"+pnt.getLevel()+"\t|\t"+pnt.getMasa_Kerja()+"\t\t|\t"+pnt.getStatus()+"\t\t|\t"+pnt.getGaji_Pokok()+"\t\t|\t"+pnt.getJumlah_Lembur()+"\t\t|\t"+pnt.getJumlah_Cuti()+"\t\t|\t"+pnt.getJumlah_Absen()+"\t\t|\t"+pnt.getJumlah_Dinas()+"\t\t|");
			System.out.println("     ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		}
	...
	public static void FormSlipGaji(List<PegawainonTetap> listPegawainonTetap) {
		...
		IDP = s.nextLine();
		pegawainonTetap.setIDPnT(IDP);
		for (PegawainonTetap pnt : listPegawainonTetap) {
			if(IDP.equals(pnt.getIDPnT())){
				int bonus = pnt.getJumlah_Dinas() * 150000;
				int potongan = pnt.getJumlah_Cuti() * 200000;
		        ...
		        System.out.println("              | IDP			: " +pnt.getIDPnT());
